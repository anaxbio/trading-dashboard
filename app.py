...
def run_engine(threshold, use_sma_wall):
    ...
    if not df.empty:
        # Ranking Logic (Shared by both tabs)
...
with tab1:
    ...
    if 'intra_results' in st.session_state and not st.session_state.intra_results.empty:
        ...
        # Position Sizer: 5X leverage allows 1,00,000 exposure per 20,000 margin
        df_i['Max_Qty_5X'] = (100000 / df_i['LTP']).astype(int)
        st.table(df_i[['Rank', 'Symbol', 'LTP', 'Max%', 'Max_Qty_5X']])
    ...
    def live_intra():
        st.caption(f"Next Sync: {get_now_ist().strftime('%H:%M:%S')}")
        try:
...
with tab2:
    ...
    if 'swing_results' in st.session_state and not st.session_state.swing_results.empty:
        ...
        st.table(df_s[['Rank', 'Symbol', 'LTP', 'Dist_Wall%', 'Qty_for_20k']])
        
        with st.form("swing_commit"):
            confirmed_s = []
            for i, row in df_s.iterrows():
                if row['Rank'] == "🔥 LEADER":
                    if st.checkbox(f"Invest ₹20k in {row['Symbol']} ({row['Qty_for_20k']} shares)", key=f"sw_{row['Symbol']}"):
